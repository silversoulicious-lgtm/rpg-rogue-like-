...
> `scripts/Hud.gd` + `scripts/Ui.gd` (icons / 9-patch), plus the combat/spell
> VFX visual language (§10 — consumed by guide Phase 3.8 and 6.9). No
> gameplay logic: VFX are display-only replays of resolutions that already
> happened.
...
## 10. Combat & spell VFX — the visual language

The game already resolves chain lightning (`bounce_attack`, Main.gd:998, used
by the `chain_lightning` skill), piercing shots, AoE and ranged attacks — all
**instantaneously and invisibly** (log text only). This section defines how
those resolutions get *drawn*. Guide Phase 3.8 wires the hooks; this section
owns the pixels.

Two kinds of work, keep them separate in your head:
- **Draw-time** (most of it): procedural polylines/pixels in `MapView._draw`,
  zero PNGs, animated per frame. §10.1–10.2.
- **Build-time** (a little): small projectile/spark sprites generated by
  `_assets_gen.gd` like any other asset. §10.3.

### 10.0 Architecture (MapView, display only)

- `_world_fx: Array[Dictionary]` — entries `{id, kind, t0, dur, data}`.
  Spawned by `fx_*` calls from Main, drawn in `_draw` **after** entities and
  floaters, pruned in `_process` when `_anim_t - t0 > dur`. This is a sibling
  of the existing `_fx` per-entity layer (MapView.gd:44), not a replacement.
- **Logic is never delayed.** Main resolves everything instantly, then hands
  MapView a *description* of what happened (the hit path, the center, the
  element). FX are fire-and-forget replays. If the player queues the next
  input mid-bolt, the bolt keeps playing over the new state — that's fine.
- **RNG discipline**: FX jitter must come from a **local**
  `RandomNumberGenerator` seeded from the fx id — never `Main.rng` (consuming
  gameplay rolls for visuals breaks seeded runs, guide 3.7) and never the
  generator's build-time `rng`. For frame-stable crackle, reseed as
  `hash(fx.id * 1000 + int((_anim_t - t0) / 0.05))` — deterministic per
  crackle-frame, frame-rate independent.
- **Cap**: 24 live world-fx; on overflow drop the oldest. Keeps `_draw` cost
  bounded no matter how proc-happy a build gets.
- **Element → color table** (identity palette only, no new colors):

  | element | core | halo/outer |
  |---|---|---|
  | foudre | CYAN, kinks in white | ARCANE (α 0.22 wide pass) |
  | feu | EMBER lightened | BLOOD darkened (α 0.20) |
  | givre | white | CYAN (α 0.22) |
  | venin | POISON lightened | POISON darkened (α 0.25) |
  | arcane | ARCANE_L | ARCANE (α 0.22) |
  | physique | BONE | INK_SOFT (α 0.3) |

### 10.1 `fx_bolt(path: Array[Vector2i], elem := "foudre")` — the electric arc (flagship)

Input: the hit chain in order — `[origin_cell, victim_0, victim_1, …]`
(Main's `bounce_attack` collects exactly this while it iterates). Rendering:

1. Convert cells to pixel centers (the same centers entities draw at, so the
   arc visually connects sprites, not tiles).
2. **Stagger — the chain must read as travel**: segment *i*
   (victim *i-1* → victim *i*) becomes visible at `t0 + i * 0.06 s`; total
   `dur = 0.30 + 0.06 * segment_count`. The logic already happened; only the
   *reveal* is staggered.
3. **Jagged polyline** per segment: subdivide every 5–6 px; offset each
   interior point perpendicular to the segment by ±3 px (local rng),
   endpoints pinned to the entity centers. **Re-roll the jitter every
   0.05 s** (the crackle) using the reseed rule from §10.0 — the bolt shape
   flickers but is deterministic per frame-window.
4. **Draw passes** per visible segment, in order:
   a. wide soft halo — `draw_polyline`, width 5, halo color;
   b. core — width 2, core color;
   c. 2–3 single white pixels at the sharpest kinks (pick every other
      interior point; no angle math needed).
5. **Branches**: per segment, 35 % chance of one fork — 4–7 px, width 1,
   core color at α 0.5, from a random kink at roughly ±45°, lives for one
   crackle-frame only. This is what makes it read as electricity rather
   than a zigzag line.
6. **Impact per victim**, at that victim's segment-arrival time: the
   existing white hit flash (`fx_hit` gains an optional `delay` parameter,
   guide 3.8) + a 4-armed pixel star (arms 3→5 px over 0.12 s, core white,
   tips core-color).
7. **Fade**: global alpha `(1 - t/dur)` multiplied by flicker
   `0.75 + 0.25 * sin(t * 70.0)`.

The same function with `elem = "arcane"` etc. covers any future chain effect
for free — element only swaps the color row.

### 10.2 The rest of the kit (same architecture, one dict-kind each)

| fx | Visual | dur | Used by (guide refs) |
|---|---|---|---|
| `fx_beam(from, to, elem)` | straight bolt: same 3 passes as 10.1 but **no jitter** on the core (1 px jitter on the halo only), no stagger — a pierce is instantaneous by nature | 0.18 s | `pierce_attack` (3.8) |
| `fx_projectile(from, to, kind)` | a generated sprite (§10.3) travels the line at ~480 px/s, rotated to direction; spawns a 3-px spark star on arrival | distance-based | `_enemy_ranged_attack`, `_enemy_cast`, player bow/orb skills (3.8) |
| `fx_burst(center, elem, radius)` | expanding 1-px circle outline (cell-radius → radius tiles over the dur) + 8–12 spark pixels flying outward ballistically (slight downward pull), halo disc α 0.15 behind | 0.35 s | `aoe_attack`, `explosive`, `ai.explode`, nova procs (3.8, 6.9) |
| `fx_slash(pos, dir)` | 3-px-wide arc (quarter circle) swept in front of `pos` toward `dir`, BONE→white, 2 crackle-frames | 0.12 s | melee multi-hit skills |
| `fx_heal(pos)` | 5 gold sparkle pixels rising 10 px, staggered starts, twinkle (alpha sine) | 0.5 s | heal sites (potion, lifesteal, regen ≥3) |
| `fx_cloud(pos, elem)` | 3 overlapping soft discs (α 0.18) drifting apart 4 px, venin color | 0.6 s | poison procs, Phase 4 gas tiles |

Budget rule: each fx kind ≤ ~40 lines of draw code. If one wants more, it's
over-designed — cut, don't polish.

### 10.3 Generated FX sprites (build-time)

Standard generator work — §0.1 reseed rule (seed = filename), auto-included
in contact sheets since they're saved sprites:

- `fx_arrow` — 8×3: BONE shaft, STEEL head, 2 ROSE fletch pixels.
- `fx_orb_feu`, `fx_orb_givre`, `fx_orb_venin`, `fx_orb_arcane` — 6×6:
  2-px core in the element's core color + `_glow` halo in the halo color.
- `fx_spark_f0` / `fx_spark_f1` — 5×5 4-armed star, frames = arms 3 px / 5 px
  (reused by bursts and projectile impacts as a cheap animated impact).

MapView loads them with the normal texture path (`_load_textures` probe);
missing PNG → fall back to a 2×2 colored rect so the FX layer never
hard-depends on the generator run.

### 10.4 Readability & restraint rules

- Draw order: terrain → entities → world-fx → floaters (damage numbers) →
  atmosphere/vignette. Damage numbers must stay readable *over* the flashiest
  bolt.
- **No full-screen flashes, ever** (photosensitivity). Impacts flash the
  victim's cell only; the existing screenshake setting is respected and a
  `settings.reduced_fx` toggle (guide 1.8's pause/options surface) skips
  branches/sparks and halves durations — bolts/bursts still appear (they are
  *information*: who got hit), just calmer.
- FX may cross unexplored cells (a mid-chain segment can pass over fog);
  acceptable in v1 — victims themselves are always visible because targeting
  requires visibility (guide 2.2). Note it, don't clip it.
- One vocabulary: every element uses the same core/halo/spark grammar with
  swapped colors. A new effect that needs a new *shape* (not a new color) is
  a design decision, not a default.

---

## 11. Execution order & checkpoints

Each checkpoint = regenerate → import pass → contact sheets → one in-game
xvfb screenshot → smoke test green → commit (sheets included).

Scheduling note: §10 (combat VFX) is NOT on this ladder — it ships with
guide **Phase 3.8 / 6.9**, long before Phase 8. Only its build-time sprites
(§10.3) touch the generator; generate them under the §0.1 reseed rule
whenever 3.8 lands (C1 makes their diffs clean but is not a prerequisite).
